 ...
 */
public class ESplashVideoManagerActivity extends BaseActivity {
    private static final String TAG = "ESplashVideoManagerActivity";
    private static final String AD_ID = "3267687332";//视频摇一摇
    ...
    private boolean mIsPreloadLabelHidden;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.demo_e_video_splash_manager_activity);
    ...
    private void initView() {
        ...
        headBarLayout.setTitle(R.string.e_video_splash_title);
        ...
        initScaleView(); //默认选择 1080 * 1920 广告图片
        ...
        mAdLoadBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                expressViewWidthDp = (int) (mSeekWidthBar.getProgress() * 1.0f / mSeekWidthBar.getMax()
                        * ScreenUtils.getScreenWidthDip(ESplashVideoManagerActivity.this));
                expressViewHeightDp = (int) (mSeekHeightBar.getProgress() * 1.0f / mSeekHeightBar.getMax()
                        * ScreenUtils.getRealScreenHeight(ESplashVideoManagerActivity.this));
                ...
                //线程选择
                if (ThreadChooseUtils.isMainThread(ESplashVideoManagerActivity.this)) {
    ...
    private void loadAdAndShow(String slotId, float expressViewWidthDp, float expressViewHeightDp) {
        ...
        JADSlot slot =
                new JADSlot.Builder()
                        ...
                        .setPreloadLabelHidden(mIsPreloadLabelHidden)//是否隐藏'已预加载'标签,默认不隐藏
                        .build();

        //Step2: 创建JadSplash，参数包括广告位参数和回调接口
        mJADSplash = new JADSplash(ESplashVideoManagerActivity.this, slot);
        // 设置视频广告相关接口回调
        mJADSplash.setSplashVideoListener(new JADSplashVideoListener() {
            @Override
            public void onPlayerStatusChanged(int playerStatus) {
                logI(getString(R.string.ad_video_player_status, AD_TAG, playerStatus));
            }

            @Override
            public void onPlayerError(int code, String error) {
                logI(getString(R.string.ad_video_player_failed, AD_TAG, code, error));
                removeSplashAd();
                mAdLoadBtn.setEnabled(true);
                mAdLoadBtn.setText(getString(R.string.ad_video_player_failed, AD_TAG, code, error));
            }
        });
        //Step3: 加载广告
        mJADSplash.loadAd(new JADSplashListener() {
            ...
            public void onLoadFailure(int code, String error) {
                ...
                new DemoDialog(ESplashVideoManagerActivity.this, "Error LoadFailed", "错误码：" + code + "\n" + "错误信息：" + error, new DemoDialog.dialogCallback() {
                    @Override
    ...
    private void resetDes(int progress, SeekBar seekBar, TextView desTv, boolean isWidth) {
        ...
        if (hProgress == 0) {
            new DemoDialog(ESplashVideoManagerActivity.this, "Error", "0不可以做分母", new DemoDialog.dialogCallback() {
                @Override
                public void dismissCallback() {
                    initScaleView();
                }
