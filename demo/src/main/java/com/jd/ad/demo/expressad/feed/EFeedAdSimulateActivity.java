...
public class EFeedAdSimulateActivity extends BaseActivity {
    ...
    //当前点击的广告实例
    private FeedAdWrapper mClickFeedAdWrapper;
    ...
    private void createADView(float width, float height, int position) {
        ...
        new FeedAdWrapper(EFeedAdSimulateActivity.this, createJADSlot(width, height))
                .loadAd(new FeedAdWrapper.OnAdLoadListener() {
                    ...
                    @Override
                    public void onAdClick(FeedAdWrapper feedAdWrapper) {
                        mClickFeedAdWrapper = feedAdWrapper;
                    }
                });
    }
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == JADYunSdk.REQUEST_CODE_MEDIA_DIALOG) {
            if (resultCode == RESULT_CANCELED) {
                ...
                if (mClickFeedAdWrapper != null && mClickFeedAdWrapper.getJADFeed() != null) {
                    mClickFeedAdWrapper.getJADFeed().onUserCancelJdJump();
                }
            }
        }
    }
